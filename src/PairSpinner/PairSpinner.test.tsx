...
describe('PairSpinner', () => {
  ...
  describe('.previousMemberPairings', () => {
    it('should return members\' history of pairs', () => {
      ...
      expect(subject.previousMemberPairings('a', [...previousPairings, ...previousPairings])).toEqual([['a', 'b'], ['a', 'b']]);
    });
  });
  ...
  describe('.newPairCombinations', () => {
    ...
    it('should return combinations of paring that has not been done before', () => {
      expect(subject.newPairCombinations(allPossibleCombinations, oldCombinations)).toEqual(['a', 'c']);
    });
  ...
  describe('.removePairFromTeamMembers', () => {
    it('should remove selected pair from teamMembers array', () => {
      (expect(subject.removePairFromTeamMembers(['a', 'b'], teamMembers))).toEqual(['c', 'd']);
    });
  });

  describe('.spin', () => {
    it('should return an array of new shuffled pairs', () => {
      const actual = subject.spin(teamMembers, previousPairings).flatMap(r => r).sort();
